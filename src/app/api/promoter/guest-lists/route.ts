// src/app/api/promoter/guest-lists/route.ts
import { NextResponse, type NextRequest } from 'next/server';
import { createClient } from '@/utils/supabase/server';

// GET /api/promoter/guest-lists - Get all guest lists for a promoter with event info and stats
export async function GET(request: NextRequest) {
  ...
  // For now, get promoter_user_id from query param (will use authenticated user later)
  const searchParams = request.nextUrl.searchParams;
  const promoterUserId = searchParams.get('promoter_user_id');

  if (!promoterUserId) {
    return NextResponse.json(
      { error: 'promoter_user_id query parameter required' },
      { status: 400 }
    );
  }

  try {
    // Fetch all guest lists created by this promoter with event data
    const { data: guestLists, error: listsError } = await supabase
      ...
      `)
      .eq('created_by_user_id', promoterUserId)
      .eq('list_type', 'promoter_list')
    ...
    console.error('Error in promoter guest lists API:', error);
    return NextResponse.json(
      {
