// src/app/api/guest-lists/[id]/approve/route.ts
import { NextResponse, type NextRequest } from 'next/server';
import { createClient } from '@/utils/supabase/server';

// POST /api/guest-lists/[id]/approve - Approve one or multiple guest entries
export async function POST(
  ...
  if (profileError || !profile) {
    console.error('Error fetching profile for approval:', profileError);
  ...
  // Only list creator, managers, and owners can approve guests
  if (
    ...
    return NextResponse.json(
      { error: 'Forbidden: Only list creators, managers, and owners can approve guests.' },
      { status: 403 }
    );
  }
  */
  ...
  // 4. Update Guest List Entries to approved
  ...
  if (updateError) {
    console.error('Error approving guest list entries:', updateError);
    return NextResponse.json(
      { error: 'Failed to approve guest list entries.', details: updateError.message },
      { status: 500 }
    );
  }
  ...
  return NextResponse.json(
    {
      message: `${updatedEntries?.length || 0} guest(s) approved successfully.`,
